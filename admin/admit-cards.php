...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    if ($id) {
        ...
        redirect('admit-cards.php?deleted=1');
    }
}
...
?>
            ...
            <div class="flex flex-col md:flex-row gap-4">
                <div class="flex-1">
                    <input type="text" id="search-input" value="<?= htmlspecialchars($search ?? '') ?>" 
                           placeholder="Search admit cards..." class="form-input">
                </div>
                <div>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-bold text-gray-800">Admit Cards (<?= $totalAdmitCards ?> total)</h2>
                        ...
                        <tr>
                            <td>
                                <div class="font-medium text-gray-800 line-clamp-2">
                                    <?= htmlspecialchars($admitCard['title']) ?>
                                </div>
                                <div class="text-sm text-gray-500">
                                    <?= timeAgo($admitCard['published_at']) ?>
                                </div>
                            </td>
                            ...
                            <td>
                                <span class="badge <?= $admitCard['status'] === 'published' ? 'badge-success' : 'badge-warning' ?>">
                                    <?= ucfirst($admitCard['status']) ?>
                                </span>
                            </td>
                            <td>
                                <div class="flex gap-2">
                                    <a href="<?= SITE_URL ?>/admit/<?= urlencode($admitCard['slug']) ?>" target="_blank"
                                       class="text-blue-600 hover:text-blue-800" title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="edit-admit-card.php?id=<?= $admitCard['id'] ?>"
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50">
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
                ...
                <p class="text-gray-600 mb-6">Are you sure you want to delete "<span id="deleteAdmitCardTitle"></span>"?</p>
                
                <form id="deleteForm" method="POST">
                    <input type="hidden" name="action" value="delete">
                    <input type="hidden" name="id" id="deleteAdmitCardId">
    ...
    <script>
        ...
        function deleteAdmitCard(id, title) {
            document.getElementById('deleteAdmitCardId').value = id;
            document.getElementById('deleteAdmitCardTitle').textContent = title;
